    ...
    def chunk_document(self, doc, *, document_id: str, metadata: Dict[str, Any] | None = None) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            def _emit_table(tbl) -> None:
                ...
                # --- LOCAL PATCH (project117) -----------------------------
                # A spreadsheet sheet is a single Docling table. Emitting it
                # atomically produced one ~20k-token chunk for the Excel
                # register, which the embedding model rejected (Ollama 500),
                # failing the whole document. Split oversized tables into
                # row-packed chunks that respect max_tokens, repeating the
                # Markdown header row so each chunk stays self-describing.
                if _token_len(tbl_md) <= self.max_tokens:
                    _add_chunk(tbl_md, "table", heading_path=current_heading_path[:], page_no=_page_of(tbl))
                    return
                lines = tbl_md.splitlines()
                header = lines[0] if lines and lines[0].lstrip().startswith("|") else ""
                sep = ""
                if header and len(lines) > 1:
                    bare = lines[1].replace("|", "").replace(" ", "")
                    if bare and set(bare) <= {"-", ":"}:
                        sep = lines[1]
                prefix = f"{header}\n{sep}\n" if header and sep else (f"{header}\n" if header else "")
                body = lines[2:] if (header and sep) else (lines[1:] if header else lines)
                page_no = _page_of(tbl)
                pending: List[str] = []
                for line in body:
                    if pending and _token_len(prefix + "\n".join(pending + [line])) > self.max_tokens:
                        _add_chunk(
                            prefix + "\n".join(pending),
                            "table",
