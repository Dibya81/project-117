...
class DocumentConverter:
    ...
    SUPPORTED_FORMATS = {
        ...
        # Upstream omitted the Office formats, so PPTX/XLSX uploads validated
        # but converted to nothing ("No text chunks were generated"). Docling
        # maps both natively (PowerpointFormatOption/ExcelFormatOption, applied
        # by the default general converter above), so no extra pipeline options
        # are required. Legacy binary .ppt/.xls are deliberately NOT listed:
        # they route through a LibreOffice conversion step that is not
        # guaranteed to be present, so we do not claim support for them.
        # ------------------------------------------------------------------
